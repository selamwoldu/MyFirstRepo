$(()=>{
 ...
 $('#btn').on('click',(event)=>{
    ...
    if(val=='deposit'){
      $.post("http://localhost:5007/deposit",{name:inname,number:innumber,balance:inbalance})
      .done((data)=>{
         showResult(data);
      }).fail((err)=>{
   ...
   $.post("http://localhost:5007/withdraw",{name:inname,number:innumber,balance:inbalance})
   .done((data)=>{
   ...
   $.post("http://localhost:5007/balance",{number:innumber})
   .done((data)=>{
      showbalance(data);
   }).fail((err)=>{
       console.log('error occured'+err);
   ...
   $.post("http://localhost:5007/createCustomer",{name:inname,number:innumber,balance:inbalance})
   .done((data)=>{
      createCustomer(data);
   }).fail((err)=>{
       console.log('error occured'+err);
